...
def dijkstra(
    ...
    ignored_nodes: list[Zone] | None = None,
    ignored_edges: list[tuple[Zone, Zone]] | None = None
    ...
    while True:
        ...
        for i in range(len(last_winner_path) - 1):
            ...
            spur_cost, spur_path = dijkstra(
                graph, spur_node, end, ignored_nodes, ignored_edges
            )
            ...
            if spur_path:  # If Dijkstra actually found a way out
                ...
                if (
                    new_candidate not in The_Waiting_Room
                    and new_candidate not in The_Winner
                ):
